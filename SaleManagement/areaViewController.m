...
//  SaleManagement
//
//  Created by chaiyuan on 16/6/16.
...
FX_Button *depBtn;//部门排行
FX_Button *comBtn;//公司排行
FX_Button * dayButton;
...
-(void)viewWillAppear:(BOOL)animated{
    ...
    _requestDic[@"type"]=[NSNumber numberWithInteger:2];
...
-(void)makeView{
    ...
    [self addNavgationbar:@"区域内排行榜" leftImageName:nil rightImageName:nil target:self leftBtnAction:nil rightBtnAction:nil leftHiden:NO rightHiden:YES];
    
    //头部点击操作区域-- 个人排名--部门内排名---公司内排行
    ...
    renBtn = [[FX_Button alloc] initWithFrame:CGRectMake(0, 0, __MainScreen_Width/3, SelectViewHeight+2-0.8) andType:@"4" andTitle:[typeA objectAtIndex:0] andTarget:self andDic:nil];
    renBtn.tag = 0;
    renBtn.isSelect = YES;
    ...
    depBtn = [[FX_Button alloc] initWithFrame:CGRectMake(__MainScreen_Width/3, 0, __MainScreen_Width/3, SelectViewHeight+2-0.8) andType:@"4" andTitle:[typeA objectAtIndex:1] andTarget:self andDic:nil];
    depBtn.tag = 1;
    [handV addSubview:depBtn];
    
    
    comBtn = [[FX_Button alloc] initWithFrame:CGRectMake(__MainScreen_Width/3*2, 0, __MainScreen_Width/3, SelectViewHeight+2-0.8) andType:@"4" andTitle:[typeA objectAtIndex:2] andTarget:self andDic:nil];
    comBtn.tag = 2;
...

{
    ...
    if (renBtn.isSelect == YES) {//个人排名
    ...
    }else if (depBtn.isSelect == YES){//部门排名
        ...
        [FX_UrlRequestManager postByUrlStr:AreaDept_url andPramas:_requestDic andDelegate:self andSuccess:@"CountryPersonalSuccess:" andFaild:@"CountryPersonalFild:" andIsNeedCookies:YES];
        ...
        [FX_UrlRequestManager postByUrlStr:AreaSub_url andPramas:_requestDic andDelegate:self andSuccess:@"CountryDeptSuccess:" andFaild:@"CountryPersonalFild:" andIsNeedCookies:YES];
    }
}

#pragma mark ---- 个人排名、分司排名\分司排名
-(void)changeBigAndColorClikedBack:(FX_Button *)sender{
    ...
    if (sender.tag ==0) {//个人排名
        ...
        depBtn.isSelect = NO;
         [depBtn changeBigAndColorCliked:depBtn];
        comBtn.isSelect = NO;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (_myClientTable == tableView) {
        ...
        NSDictionary *dataDic = [_tableArr objectAtIndex:indexPath.row];
        cell1.nameL.text = [ToolList changeNull:[dataDic objectForKey:@"subName"]];
