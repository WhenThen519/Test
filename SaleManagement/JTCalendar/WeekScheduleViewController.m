...
#pragma mark - 总监根据传入日期和传入ID,查询该部门该天的日程请求成功
-(void)requestSuccess1:(NSDictionary *)resultDic
{
    isBumenLook = NO;
    ...
    {
        [ToolList showRequestFaileMessageLittleTime:@"暂无部门日程数据"];
        if(startPage == 1)
        {
            [dataArr removeAllObjects];
            [table reloadData];
...
#pragma mark - 总监根据传入日期和传入ID,查询该部门该天的日程
-(void)lookBumen
{
   NSMutableDictionary* reqDicD = [[NSMutableDictionary alloc] init];
    [reqDicD setObject:@"10" forKey:@"pagesize"];
    [reqDicD setObject:selectDate?selectDate:requestDate forKey:@"date"];
    [reqDicD setObject:deptId?deptId:@"" forKey:@"deptId"];
    [reqDicD setObject:[NSString stringWithFormat:@"%d",startPage] forKey:@"pageNo"];
    [FX_UrlRequestManager postByUrlStr:Schedule4Sub_url andPramas:reqDicD andDelegate:self andSuccess:@"requestSuccess1:" andFaild:nil andIsNeedCookies:YES];
...
-(void) requestAlldata
{
    ...
    [FX_UrlRequestManager postByUrlStr:ScheduleByDate4Sub_url andPramas:reqDic andDelegate:self andSuccess:@"requestSuccess:" andFaild:nil andIsNeedCookies:YES];
...
- (void)viewDidLoad {
    [super viewDidLoad];
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(lookBumen) name:@"shuaxin" object:nil];
    startPage = 1;
    isSelected = NO;
    isBumenLook = YES;
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if(isBumenLook)
    {
    static NSString *cellID = @"WeekScheduleListCell";
    WeekScheduleListCell *cell = [tableView dequeueReusableCellWithIdentifier:cellID];
    if(cell==nil)
    {
        cell = [[[NSBundle mainBundle] loadNibNamed:@"WeekScheduleListCell" owner:self options:nil] lastObject];
        //线
    ...
    cell.bumen_L.text = [ToolList changeNull:[dic objectForKey:@"deptName"]];
    cell.event_L.text = [NSString stringWithFormat:@"%@ 个事件",[ToolList changeNull:[dic objectForKey:@"count"]]];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    {
        NSDictionary *dic = [dataArr objectAtIndex:indexPath.row];
        deptId = [dic objectForKey:@"deptId"];
        [self lookBumen];
    }
    else
    {
